...
/* ══════════════════════════════════════════════════════════════
   API DATA LAYER
   All persistent state is fetched from / saved to the server.
   Theme stays in localStorage (per-visitor preference only).
══════════════════════════════════════════════════════════════ */

let adminToken = sessionStorage.getItem("nc_admin_token") || null;

// Debounced full-state save — called like saveD() anywhere (ignores old key/val args)
let _saveTimer = null;
function saveD() {
	clearTimeout(_saveTimer);
	_saveTimer = setTimeout(async () => {
		if (!adminToken) return;
		// Never send the password back through the data endpoint
		...
		try {
			...
			if (res.status === 401) {
				// Session expired
				adminToken = null;
				sessionStorage.removeItem("nc_admin_token");
				logoutAdmin();
				showToast("⚠️ Session expired — please log in again.");
			}
		} catch {
			showToast("⚠️ Save failed — is the server running?");
		}
	}, 300);
}

/* ══════════════════════════════════════
   DEFAULTS  (used as in-memory fallback)
══════════════════════════════════════ */
...
const DEFAULT_TAGS = ["VRChat", "EN", "Photographer", "Music Addict", "Femboy", "Cat Boy"];

// Live DATA object — will be populated from the server on load
let DATA = {
	playlist: DEFAULT_PLAYLIST,
	statuses: DEFAULT_STATUSES,
	profile:  { name: "JustNeychii", username: "CHEEZEBORGIR", bio: "" },
...
/* ══════════════════════════════════════
   THEME  (stays per-visitor in localStorage)
══════════════════════════════════════ */
...
/* ══════════════════════════════════════
   CONTENTEDITABLE — name / username / bio
══════════════════════════════════════ */
["displayName", "usernameDisplay", "bioText"].forEach(id => {
	const el = document.getElementById(id);
	el.addEventListener("blur", () => {
		if (!isAdmin) return;
		if (id === "displayName")    DATA.profile.name     = el.textContent;
		if (id === "usernameDisplay") DATA.profile.username = el.textContent;
		if (id === "bioText")        DATA.profile.bio      = el.innerHTML;
		saveD();
		showToast("Saved!");
	});
});
...
document.getElementById("artImg").addEventListener("click", e => {
	...
	openEditModal(
		...
		DATA.images.art || "./Assets/Images/art.png",
		val => {
			if (!val.trim()) return;
...
function closeOverlay(id) {
	const el   = document.getElementById(id);
	const card = el.querySelector(".modal-card,.admin-panel,.gallery-full-img-wrap,.edit-modal-card");
	if (card) {
...
document.querySelectorAll(".overlay-bg").forEach(bg => {
	if (!bg.onclick)
		bg.addEventListener("click", e => {
			...
			if (ov && ov.id !== "adminOverlay" && ov.id !== "editModal") closeOverlay(ov.id);
		});
});
let pendingLink = "";
...
document.addEventListener("keydown", e => {
	if (e.key === "Escape")
		["socialOverlay","matureOverlay","galleryFull","adminOverlay","editModal"].forEach(id => {
			const el = document.getElementById(id);
...
/* Polaroid click */
const polaroidLabelMap = { 0: "Top-Left", 1: "Top-Right", 2: "Bottom-Left", 3: "Bottom-Right" };
...
function startHold(e) {
	if (e.target.closest(".tag,.like-chip,.status-item,.chip")) return;
	holdStart = Date.now();
	...
	function tick() {
		...
		else { holdRing.classList.remove("active"); holdFill.style.width = "0%"; openAdmin(); }
	}
...
// If we have a stored token, validate it immediately (restore session)
if (adminToken) {
	fetch("/api/data", { headers: { "Authorization": `Bearer ${adminToken}` } })
		.then(r => { if (!r.ok) { adminToken = null; sessionStorage.removeItem("nc_admin_token"); } })
		.catch(() => {});
}

function openAdmin() {
	// If already have a valid token, skip login screen
	if (adminToken) {
		isAdmin = true;
...
document.getElementById("adminBadge").addEventListener("click", () => { if (isAdmin) openAdminDash(); });
...
async function doLogin() {
	...
	try {
		...
		adminToken = token;
		sessionStorage.setItem("nc_admin_token", token);
...
function switchTab(name) {
	...
	const tabs = ["profile","images","music","statuses","gallery","likesdislikes","schedule","password"];
	document.querySelectorAll(".admin-tab")[tabs.indexOf(name)]?.classList.add("active");
	document.getElementById("tab-" + name)?.classList.add("active");
}

function loadAdminFields() {
	const p = DATA.profile, im = DATA.images, s = DATA.schedule;
	document.getElementById("editName").value      = p.name     || "";
	document.getElementById("editUsername").value  = p.username || "";
	document.getElementById("editBio").value       = (p.bio || "").replace(/<br\/>/g, "\n");
...
function saveProfile() {
	...
	DATA.tags = document.getElementById("editTags").value
...
function saveGalleryCounts() {
	DATA.gallery.sfw  = parseInt(document.getElementById("editSfwCount").value)  || 23;
	DATA.gallery.nsfw = parseInt(document.getElementById("editNsfwCount").value) || 8;
	// Update galleryConfig live
...
function addTrack() {
	...
	if (!title || !src) { showToast("Title and URL required!"); return; }
...
async function changePassword() {
	...
	try {
		...
		showToast("⚠️ Server error changing password.");
	}
}
...
function closeEditModal() { closeOverlay("editModal"); editModalCb = null; }
...
/* ══════════════════════════════════════
   PAGE INIT  — fetch server data, then render
══════════════════════════════════════ */
...
async function initPage() {
	...
	try {
		...
		showToast("⚠️ Couldn't reach server — showing defaults");
	}

	// Sync live variables that alias DATA arrays
	statuses = DATA.statuses;
	playlist = DATA.playlist;
	galleryConfig.sets.default.count = DATA.gallery.sfw;
	galleryConfig.sets.nsfw.count    = DATA.gallery.nsfw;
